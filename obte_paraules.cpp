...
#include "word_toolkit.hpp"
  ...
  static void insert_in_order_no_dup(list<string>& L, const string& w) {
    // Reemplazamos auto por list<string>::iterator
    ...
    // Si no hemos llegado al final y *it == w, no insertamos (duplicate).
  ...
  // Comparador para luego, si se desea, ordenar la lista por (longitud, alfabético).
  /*static bool cmp_len_then_lex(const string& a, const string& b) {
...
void obte_paraules::obte_paraules(const string& s, const anagrames& A, list<string>& paraules) throw(error) {
    ...
    // 2) Lista global: irá en orden alfabético, luego la reordenamos por longitud
    list<string> paraules_global;
